...
1. Нельзя удалить текущую ветку.
...
**Примечание:** Команды `git checkout` считаются устаревшими для операций с ветками. Вместо них рекомендуется использовать современные команды `git switch` и `git restore`.
...
'''
→ ORIG_HEAD — это указатель на коммит до последнего merge, а не произвольный hash.

→ git reset --merge <hash> не поддерживается напрямую так — опция --merge работает только как git reset --merge (без аргументов) или вместе с --merge внутри git merge (но это не так).
